...
class Map_Obj():
    ...
    def read_map(self, path):
        ...
        df = pd.read_csv(path, index_col=None, header=None, delimiter=",")  # ,error_bad_lines=False)
...
def astar(maze, start, goal):
    ...
    while len(openList) > 0:
        ...
        for new_position in [(0, -1), (0, 1), (-1, 0), (1, 0)]:
            # print("current position [0]: ", current_node.position[0], "new node position[0]: ", new_position[0])
            # print("current position [1]: ", current_node.position[1], "new node position[1]: ", new_position[1])
            node_position = (current_node.position[0] + new_position[0], current_node.position[1] + new_position[1])
            # Check if new pos is inside maze
        ...
        for child in children:
            ...
            # Check if child is already discovered and if the already discoved node position has higher g value
...
def main():
    ...
    listmaze = maze.tolist()
    # Prints the maze that astar runs on
    # print(listmaze)

    # Find shortest path from start to goal
    path = astar(listmaze, tuple(start), tuple(goal))
    print(path)

    # ------------------------------------------
    # Comment out the above code and run test code with simpler mazes
